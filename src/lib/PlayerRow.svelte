<script>
  ...
  const medalColor = player.rank === 1 ? '#f59e0b'
    : player.rank === 2 ? '#e2e8f0'
    : player.rank === 3 ? '#b87333'
    : null;

  const changeLabel =
    player.rankChange > 0 ? `↑${player.rankChange}` :
    player.rankChange < 0 ? `↓${Math.abs(player.rankChange)}` : null;
...
{#if player.rank === 1}
  <div class="rounded-2xl mb-4 border cursor-pointer"
    ...
    <div class="flex items-center gap-4 px-5 py-4">
      ...
      {/if}
      <span class="text-base font-bold tabular-nums" style="color: {accent}">{player.totalPoints}</span>
    </div>
    {#if expanded}
...
{:else if player.rank <= 3}
  <div class="rounded-xl mb-2 border border-slate-800 cursor-pointer"
    style="background: {medalColor}12"
    onclick={() => expanded = !expanded}>
    <div class="flex items-center gap-3 px-4 py-3">
      <span class="text-xl font-bold w-6 text-center shrink-0" style="color: {medalColor}">{player.rank}</span>
      <span class="flex-1 font-semibold text-slate-200">{player.name}</span>
      {#if changeLabel}
        <span class="text-xs font-semibold {changeClass}">{changeLabel}</span>
      {/if}
      <span class="text-sm font-semibold tabular-nums" style="color: {medalColor}">{player.totalPoints}</span>
    </div>
    {#if expanded}
...
{:else}
  <div class="rounded-lg cursor-pointer hover:bg-slate-900 transition-colors"
    onclick={() => expanded = !expanded}>
    <div class="flex items-center gap-3 px-4 py-2.5">
      <span class="w-6 text-right text-slate-600 font-mono text-sm shrink-0">{player.rank}</span>
      <span class="flex-1 text-slate-300">{player.name}</span>
      {#if changeLabel}
        <span class="text-xs font-semibold {changeClass}">{changeLabel}</span>
      {/if}
      <span class="text-sm tabular-nums text-slate-400">{player.totalPoints}</span>
    </div>
    {#if expanded}
      <div transition:slide class="px-4 pb-3">
        {@render SessionBreakdown(playedSessions, accent)}
    ...
    {#each sessions as s}
      <div class="flex items-center gap-3 text-xs text-slate-400">
        ...
        <span class="flex gap-1">
          {#each s.games as g}
            <span title={g ? 'Win' : 'Loss'} style={g ? `color: ${accent}` : ''}>
              {g ? '●' : '○'}
            </span>
          {/each}
        </span>
